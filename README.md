...
В сессии 1 блокировки таблицы в режиме RowExclusiveLock и транзакции в режиме ExclusiveLock, все в статусе granted, всё предоставлено

relation      | pg_locks  |         |         | AccessShareLock  | t  - это лёгкая блокировка pg_lock
virtualxid    |           | 4/5509  |         | ExclusiveLock    | t  - из лекции - Транзакция всегда удерживает исключительную (ExclusiveLock) блокировку собственного номера, а данном случае — виртуального.
relation      | test_text |         |         | RowExclusiveLock | t  - блокировка построчная, для строки из таблицы
...
Нет, думаю что не могут, первая всё равно выполнится. 
